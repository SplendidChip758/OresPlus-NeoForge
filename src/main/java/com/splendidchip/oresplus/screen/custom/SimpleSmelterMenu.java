...
public class SimpleSmelterMenu extends AbstractContainerMenu {
    public final SimpleSmelterBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public SimpleSmelterMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public SimpleSmelterMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.SIMPLE_SMELTER_MENU.get(), containerId);
        this.blockEntity = (SimpleSmelterBlockEntity) entity;
        ...
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 2, 56, 53){  // fuel
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.getBurnTime(ModRecipes.SIMPLE_KILN_TYPE.get(), level.fuelValues()) > 0;
            }
        });
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 3, 116, 35){
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false;
            }
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.SIMPLE_SMELTER_BLOCK.get());
    }
